...
bool hash_guardar(hash_t *hash, const char *clave, void *dato) {
  ...
  hash->tabla[pos_tabla].valor = dato;
	...
	double factor_hash = (double)hash->cant/(double)hash->tam;
 	if(factor_hash >= MAX_FACTOR) redimensionar(hash, (size_t)((double) hash->tam*FACTOR_AGRANDAR));
	return true;
}
...
void *hash_borrar(hash_t *hash, const char *clave) {
  ...
  while(pos_tabla < hash->tam && hash->tabla[pos_tabla].estado != VACIO) {
    if(hash->tabla[pos_tabla].estado == BORRADO) (pos_tabla)++;
      ...
      double factor_hash = (double) hash->cant / (double) hash->tam;
      if(factor_hash <= MIN_FACTOR && hash->tam > TAM_HASH) redimensionar(hash, (size_t)((double)hash->tam*FACTOR_REDUCCION));
